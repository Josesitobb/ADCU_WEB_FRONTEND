...
export default function TablaUsuarios() {
  ...
  const actualizarUsuario = async () => {
    try {
      ...
      await axios.put(
        `http://localhost:3000/api/users/${idEditando}`,
        datosActualizados
      );
      setMostrarModal(false);
      setModoEdicion(false);
  ...
  const confirmar = window.confirm('¿Estás seguro de eliminar este usuario? Esta acción no se puede deshacer.');
        ...
        {cargando ? (
          ...
          <div className="table-responsive">
            <Table hover className="mb-0">
              ...
              <tbody>
                {usuarios.filter(user => user.role === 'admin').map((user) => (
                  <tr key={user._id} className="align-middle">
                    ...
                    <td>{user.post || '-'}</td>
                    <td>
                      <span className={`badge ${user.state === "Activo" 
  ...
  <Modal show={mostrarModal} onHide={() => setMostrarModal(false)} centered size="lg">
    <Modal.Header closeButton className="border-0 pb-0">
      <Modal.Title className="fw-semibold">
        {modoEdicion ? "Editar Administrador" : "Registrar Nuevo Administrador"}
      </Modal.Title>
    </Modal.Header>
          ...
          <Col md={6}>
            <Form.Group controlId="password">
              ...
              <Form.Control
                ...
                value={form.password}
                onChange={handleChange}
                placeholder={modoEdicion ? "Dejar vacío para no cambiar" : "Ingrese contraseña"}
              />
            </Form.Group>
          </Col>
