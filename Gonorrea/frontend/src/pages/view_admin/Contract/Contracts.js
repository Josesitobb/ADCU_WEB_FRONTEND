...
export default function Contracts() {
  ...
  const crearContrato = async () => {
    try {
      await axios.post(`${API_URL}/Contracts`, form);
      ...
      alert("Error al crear contrato", err);
    }
  };
  // Modal para crear
  ...
  const actualizarContratos = async () => {
    try {
      ...
      await axios.put(`${API_URL}/Contracts/${idContrato}`, datosActualizados);
      setMostrarModal(false);
      setModoEdicion(false);
      setIdContrato(null);
      obtenerCotratos();
    } catch (err) {
      alert("Erro al actualizar el contratos", err);
    }
  };
  ...
  const eliminarContrato = async (idContrato) => {
    ...
    if (!confirmar) return;
    try {
      await axios.delete(`${API_URL}/Contracts/${idContrato}`);
      obtenerCotratos();
    } catch (err) {
              ...
              <tbody>
                {contracts.map((contract) => (
                  <tr key={contract._id} className="align-middle">
                    <td className="ps-4 fw-medium">{contract.typeofcontract}</td>
                    <td>{new Date(contract.starteDate).toLocaleDateString()}</td>
                    ...
                    <td>
                      <span className={`badge ${contract.state === "Activo" 
  ...
  <Modal show={mostrarModal} onHide={() => setMostrarModal(false)} centered size="lg">
    <Modal.Header closeButton className="border-0 pb-0">
      <Modal.Title className="fw-semibold">
        <i className={`bi ${modoEdicion ? 'bi-pencil-square' : 'bi-file-earmark-plus'} me-2`}></i>
      ...
      <Form>
        <Form.Group className="mb-3">
          ...
          >
            ...
            <option value="Contrato termino fijo">Contrato término fijo</option>
    ...
    <Modal.Footer className="border-0 pt-0">
      ...
      <Button 
        ...
        className="d-flex align-items-center"
      >
        <i className={`bi ${modoEdicion ? 'bi-arrow-repeat' : 'bi-save'} me-2`}></i>
